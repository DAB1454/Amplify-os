...
export default function CampaignDetailPage() {
  ...
  // Auto-generate plan when redirected from campaign creation with ?generate=true
  useEffect(() => {
    if (autoGenerate && !hasAutoGenerated.current && plan && plan.days.length === 0 && !generatingPlan) {
      ...
      setGeneratingPlan(true);
      setError(null);
      apiPost("/api/v1/ai/generate-plan", { campaign_id: campaignId })
        .then(() => fetchPlan())
        .catch((err) => setError(err instanceof Error ? err.message : "Plan generation failed"))
        .finally(() => setGeneratingPlan(false));
    }
  }, [autoGenerate, plan, generatingPlan, campaignId, router, fetchPlan]);
