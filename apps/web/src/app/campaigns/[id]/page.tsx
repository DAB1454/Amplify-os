  ...
  const handleGenerateContent = async () => {
    ...
    try {
      const result = await apiPost<{
        ...
        assets_attached: number;
        failed: number;
        skipped: number;
      }>(`/api/v1/ai/generate-content`, { campaign_id: campaignId });
      if (result.pieces_generated > 0) {
        toast.success(
          `Generated ${result.pieces_generated} caption${result.pieces_generated !== 1 ? "s" : ""}` +
          (result.assets_attached ? ` and attached ${result.assets_attached} asset${result.assets_attached !== 1 ? "s" : ""}` : "")
        );
      } else {
        toast.info("No draft posts needed content generation");
      }
      if (result.failed > 0) {
        toast.warning(`${result.failed} post${result.failed !== 1 ? "s" : ""} failed to generate`);
      }
      await fetchPlan();
    } catch (err) {
  ...
  return (
    <>
      ...
      <div className="mt-6 rounded-xl border border-[var(--border-color)] bg-[var(--bg-surface)] p-5">
        ...
        {/* Tab navigation */}
        <div className="mt-4 flex gap-1 border-b border-[var(--border-color)]">
          {(["plan", "settings"] as const).map((tab) => (
            <button
              key={tab}
            ...
            >
              {tab === "plan" ? "Plan & Posts" : "Strategy Settings"}
            </button>
          ))}
        </div>

        {/* Strategy Settings (tab: settings) */}
        {activeTab === "settings" && (
